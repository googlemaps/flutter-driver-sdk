# Contributing to Google Maps Driver Flutter Plugins
...
- **Fork the Repository:** Navigate to `https://github.com/googlemaps/flutter-driver-sdk` and create a fork in your GitHub account.
...
- **Clone Your Fork:** Use the command `git clone git@github.com:<your_name_here>/flutter-driver-sdk.git` to clone the repository to your local machine.
- **Add remote upstream:** Establish a link to the main repository for updates using command `git remote add upstream git@github.com:googlemaps/flutter-driver-sdk.git` This ensures you pull changes from the original source, not just your clone, when using git fetch and similar commands.
...
Google Maps Driver Flutter Plugins utilizes [pigeon](https://github.com/flutter/packages/tree/main/packages/pigeon) to generate the `MethodChannel` API layer between Dart and the native platforms.
...
```dart
returnValueOut = await DeliveryDriver.newMethod(parameterIn)
```
...
## 5. Running the Driver Example

The Google Maps Flutter Driver plugin provides an example app that showcases its main use-cases.

To run the Driver example, navigate to the `example` directory of the plugin and run the app:

```bash
cd example
flutter run --dart-define MAPS_API_KEY=YOUR_API_KEY --dart-define PROJECT_ID=YOUR_GOOGLE_CLOUD_PROJECT_ID
```

## 6. Running tests

Google Maps Flutter Driver package has integration and unit tests. 

### Unit tests

To run unit tests for the Google Maps Flutter Driver plugin, navigate to the plugin's root directory and execute the `flutter test` command. Use the following command:
...
Google Maps Flutter Driver integration tests can be run with the following command:

```bash
cd ./example
patrol test --dart-define MAPS_API_KEY=YOUR_API_KEY --dart-define PROJECT_ID=YOUR_GOOGLE_CLOUD_PROJECT_ID
```

To only run a specific test file, use patrol command with -t flag. For example to run a plugin_integration_test.dart run it with the following command:
...
For debugging the tests, you can add `debugPrint()` functions in your test and use patrol develop mode with the `--verbose` flag to see the printed messages. To run `plugin_integration_test.dart` in develop mode, use the following command:
...
- `git pull-request` (if you are using [Hub](http://github.com/github/hub/)) or
  go to `https://github.com/googlemaps/flutter-driver-sdk` and click the
